...
class Scanner:
    ...
    def consume(self, *expected_tokens):
        ...
        if current[0] in expected_tokens:         # tokens could be a single token, or it could be group of tokens.
            if current[0] is Token.ID or current[0] is Token.NUM: 
                #print("current[1] == " + str(current[1]))
                return current[1]
            else:                                   # otherwise
                #print("current[0] == " + str(current[0]))
                return current[0]                   # return the token
        else:                                       # if current_token is not in tokens
            self.no_token()  # raise non-token error        
